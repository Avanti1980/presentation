...
@import "../dot/odm-review-ccml.dot"
...
最大化间隔<span class="blue">均值</span>、最小化间隔<span class="red">方差</span>
...
- 选取$\ell_2$正则</li>
- 代入$\gamma (\xv_i, y_i) = y_i \langle \wv, \phi(\xv_i) \rangle$
- 拉伸$\wv$使得间隔均值$\bar{\gamma}$固定为$1$
- 引入超参数$\nu$、$\theta$增强模型的灵活性
...
由于核映射$\phi(\cdot)$的存在，内层问题通常考虑对偶形式，引入拉格朗日乘子$\alphav = [\alpha_1; \ldots; \alpha_m]$、$\betav = [\beta_1; \ldots; \beta_m]$，记$\deltav = \alphav - \betav$，根据 KKT 条件
...
$$
\begin{align*}
    \class{blue}{\min_{\yvhat \in \Bcal}} \max_{\alphav, \betav \succeq \zerov} - \frac{1}{2} \|\wv\|^2 - \underbrace{\frac{m}{2 \lambda} \begin{bmatrix}
        \alphav \\ \betav
    \end{bmatrix}^\top
    \begin{bmatrix}
        \Iv          \\
    ...
    \end{bmatrix} - \begin{bmatrix} (\theta - 1) \ev \\ (\theta + 1) \ev \end{bmatrix}^\top \begin{bmatrix}
        \alphav \\ \betav
    \end{bmatrix}}_{= A(\alphav, \betav), \text{ irrelevant to } \yvhat}
\end{align*}
$$
...
$$
\begin{align*}
    \max_{\alphav, \betav \succeq \zerov} \class{blue}{\min_{\yvhat \in \Bcal}} - \frac{1}{2} \underbrace{\deltav^\top (\Kv \odot \class{blue}{\yvhat} \class{blue}{\yvhat}^\top) \deltav}_{\|\wv\|^2} - A(\alphav, \betav)
\end{align*}
$$
...
$$
\begin{align*}
    \min_{\muv \in \Delta^{|\Bcal|}} \max_{\alphav, \betav \succeq \zerov} - \frac{1}{2} \deltav^\top \left( \sum_{t: \yvhat_t \in \Bcal} \mu_t \Kv \odot \yvhat_t \yvhat_t^\top \right) \deltav - A(\alphav, \betav)
\end{align*}
$$

现将上述 ODMSSC 的对偶问题转化为原问题
...
$$
\begin{align*}
    \min_{\muv \in \Delta^{|\Bcal|}} \max_{\alphav, \betav \succeq \zerov} & - \frac{1}{2} \deltav^\top (\class{blue}{\Kvt \odot \ev \ev^\top}) \deltav - A(\alphav, \betav) \\
    & ~ \class{red}{\bigg \Downarrow ~ \Kv \rightarrow \Kvt, ~ \yvhat \rightarrow \ev} \\
    \min_{\muv \in \Delta^{|\Bcal|}} \min_{\wvt, \xi_i, \epsilon_i} & ~ \frac{1}{2} \|\wvt\|^2 + \frac{\lambda}{2m} \sum_{i \in [m]} (\xi_i^2 + \nu \epsilon_i^2) \\
...
采用投影梯度法可求得$\alphav$、$\betav$，之后进一步可得$\epsilon_i$、$\xi_i$和$\wv_1, \ldots, \wv_{|\Bcal|}$
...
这是凸二次函数最大化的问题，很难直接求解

<div class="top2"></div>

对于第$T$轮外层循环

- 设$\muv$的非零维下标集合为$\Ical_T$
...
下证只要$\class{blue}{\yvt^\top \Hv \yvbar \neq \yvbar^\top \Hv \yvbar}$，就有$\yvt^\top \Hv \yvt > \yvbar^\top \Hv \yvbar$

反设$\yvt^\top \Hv \yvt \le \yvbar^\top \Hv \yvbar$，则有

$$
\begin{align*}
    0 & \le (\yvt - \yvbar)^\top \Hv (\yvt - \yvbar) = \yvt^\top \Hv \yvt + \yvbar^\top \Hv \yvbar - 2 \yvt^\top \Hv \yvbar \\
    & \le 2 (\yvbar^\top \Hv \yvbar - \yvt^\top \Hv \yvbar) < 0
\end{align*}
$$
...
$$
\begin{align*}
    \yvbar = \argmax_{\yvhat \in \Acal_T} \yvhat^\top \Hv \yvhat, \quad \yvt = \argmax_{\yvhat \in \Bcal} \yvhat^\top \Hv \yvbar
\end{align*}
$$
...
<div class="center threelines column4-border-right-solid head-highlight-1 tr-hover top4">

| 编号 | 名称        | #样本 | #特征 | 编号 | 名称           |  #样本 | #特征 |
| :--- | :---------- | ----: | ----: | :--- | :------------- | -----: | ----: |
| 1    | dbworld     |    64 | 3,721 | 7    | pc4            |  1,458 |    37 |
| 2    | leukemia    |    72 | 7,129 | 8    | hivaAgnostic   |  4,229 | 1,617 |
| 3    | fruitfly    |   125 |     4 | 9    | wilt           |  4,839 |     5 |
| 4    | semeion     |   319 |   256 | 10   | wilpageBlocks  |  5,473 |    10 |
| 5    | pizzaCutter |   661 |    37 | 11   | JapaneseVowels |  9,661 |    14 |
| 6    | pieChart    |   705 |    37 | 12   | letter         | 20,000 |    16 |
...
<div class="rand center threelines top0 head-highlight-1 tr-hover row12-border-bottom-solid column1-border-right-solid">
