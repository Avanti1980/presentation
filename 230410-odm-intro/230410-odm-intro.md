...
<!-- slide data-notes="尊敬的亚研院的各位专家、冯院长、各位老师，上午好，感谢刘老师 很荣幸能和高瓴的同行们分享自己的工作 我知道高瓴群英荟萃 藏龙卧虎 所以我也有点紧张 讲得不好 大家多海涵" -->
...
<!-- slide data-notes="这是大纲 我先给出必要的背景介绍 然后是我的几个工作 最后是总结"-->
...
- $\Scal = \{(\xv_1, y_1), \ldots, (\xv_m, y_m)\}$为独立同分布采样于$\Dcal$的数据集
- $\Hcal = \{ h: \Xcal \mapsto \Ycal \}$是分类器构成的假设空间，从中选择最优假设$h^\star$
...
设$\Hcal = \{ h: \Xcal \mapsto \Rbb \}$是定义在特征空间$\Xcal$上的实值函数集合，对于$\forall h \in \Hcal$，其关于样本$(\xv_i, y_i)$的{==间隔==}定义为$\gamma_i = y_i f(\xv_i)$
